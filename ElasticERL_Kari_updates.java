...
public class ElasticERL {
	...
	public ElasticERL(int size) {	//MODIFIED THIS FUNCTION!
		if (size<1000) {
			...
			hashtable = new myHashTable();	//modified this!
		}
		sizeOfERL = size;
	}
	 ...
	 * @param key
	 */
	public void remove(ElasticERL elasticERLObj, String key) {		//changed long to int here!!!!!!
		if(elasticERLObj.getSizeOfERL()<1000) {
			ElasticAVL tree = elasticERLObj.getAvltree();
	...
	/**
	 * Returns keys within a specific range
	 * @param elasticERLObj
	 * @param key1
	 * @param key2
	 * @return
